...
$id = $_GET['id'];
$qCek = mysqli_query($connect, "SELECT penduduk.*, surat_ktp_sementara.* FROM penduduk LEFT JOIN surat_ktp_sementara ON surat_ktp_sementara.nik = penduduk.nik WHERE surat_ktp_sementara.id_ks='$id'");
while ($row = mysqli_fetch_array($qCek)) {
	...
	foreach ($qTampilDesa as $rows) {
		...
		$qCekPejabatDesa = mysqli_query($connect, "SELECT pejabat_desa.jabatan, pejabat_desa.nama_pejabat_desa FROM pejabat_desa LEFT JOIN surat_ktp_sementara ON surat_ktp_sementara.id_pejabat_desa = pejabat_desa.id_pejabat_desa WHERE surat_ktp_sementara.id_pejabat_desa = '$id_pejabat_desa' AND surat_ktp_sementara.id_ks='$id'");
...
?>
			...
			<body>
				<div>
					<table width="100%">
						<tr><img src="../../../../assets/img/logo-Jepara.png="" class="logo"></tr>
						<div class="header">
						...
						<br>
						<div align="center"><u>
								<h4 class="kop">SURAT KTP SEMENTARA</h4>
							</u></div>
						<div align="center">
					...
					<div id="isi3">
						...
						<table width="100%">
							<tr>
								<td class="indentasi">Keterangan : Nama tersebut diatas benar penduduk desa kami, Kartu Tanda Penduduk (KTP) atas nama tersebut diatas sedang dalam proses. <a style="text-transform: capitalize;"> Desa <?php echo $rows['nama_desa']; ?>, Kecamatan <?php echo $rows['kecamatan']; ?>, <?php echo $rows['kota']; ?></a>. Bahwa nama yang bersangkutan tidak berada di rumah dan tidak diketahui keberadaannya.</td>
							</tr>
						</table><br>
						<table width="100%">
							<tr>
